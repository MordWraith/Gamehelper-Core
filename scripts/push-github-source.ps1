...
param(
    [string]$Repository = "MordWraith/Gamehelper-Core",
    [string]$Branch = "main",
    [string]$GitExe = "",
    [string]$Version = "",
...
function Invoke-GitCommitClean {
    param(
        [string]$Message,
        [hashtable]$Identity
    )

    $commitScript = Join-Path $PSScriptRoot "Invoke-GitCommitClean.ps1"
    if (-not (Test-Path $commitScript)) {
        throw "Fehlendes Skript: $commitScript"
    }

    & $commitScript -Message $Message -WorkingDirectory $Root `
        -AuthorName $Identity.Name -AuthorEmail $Identity.Email
    if ($LASTEXITCODE -ne 0) {
        throw "Invoke-GitCommitClean fehlgeschlagen."
    }
}
...
try {
    ...
    if ($status) {
        ...
        Invoke-GitCommitClean -Message $CommitMessage -Identity $identity
    }
    else {
    ...
    if ($LASTEXITCODE -eq 0 -and $remoteHead -and $remoteHead.Trim() -ne $localHead) {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            Invoke-GitCommitClean -Message $mergeMsg -Identity $identity
        }
    }
