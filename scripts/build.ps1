...
function Test-PluginDeploy {
    ...
    $required = @(
        @{ Plugin = "AutoHotKeyTrigger"; Files = @("AutoHotKeyTrigger.dll") },
        ...
        @{ Plugin = "PreloadAlert"; Files = @("PreloadAlert.dll") },
...
if (-not (Test-Path $Solution)) {
    ...
    # Launcher copies GameHelper.exe next to GameHelper.App.exe (must run after GameHelper output exists).
    dotnet build (Join-Path $Root "Launcher\Launcher.csproj") -c $Configuration --no-restore
    ...
    $pluginSources = Join-Path $Root "scripts\plugins-sources.json"
    if (Test-Path $pluginSources) {
        Copy-Item $pluginSources (Join-Path $PublishDir "plugins-sources.json") -Force
        Copy-Item $pluginSources (Join-Path $outDir "plugins-sources.json") -Force
    }
